      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-gray-900">{t('tasks:recurringEdit.title')}</h2>
          </div>
          <button
            onClick={onClose}
            disabled={isUpdating}
        ...
        <div className="p-4">
          <p className="text-gray-700 mb-4">
            {t('tasks:recurringEdit.partOfSeries', { title: taskTitle })}
          </p>
          ...
          <div className="space-y-3">
            ...
            >
              <div className="font-medium text-gray-900 group-hover:text-blue-700">{t('tasks:recurringEdit.editSingleTitle')}</div>
              <div className="text-sm text-gray-500 mt-1">
                {t('tasks:recurringEdit.editSingleDescription')}
              </div>
            </button>
            ...
            >
              <div className="font-medium text-gray-900 group-hover:text-purple-700 flex items-center gap-2">
                {t('tasks:recurringEdit.editAllFutureTitle')}
                <span className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xs font-medium">
                  {t('tasks:recurringEdit.editAllFutureBadge', { count: futureTaskCount })}
                </span>
              </div>
              <div className="text-sm text-gray-500 mt-1">
                {t('tasks:recurringEdit.editAllFutureDescription')}
              </div>
            </button>
          </div>
          ...
          {isUpdating && (
            <div className="mt-4 flex items-center justify-center text-gray-500">
              ...
              {t('tasks:recurringEdit.updating')}
            </div>
          )}
        </div>
